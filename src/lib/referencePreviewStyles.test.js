...
test("mobile reference preview uses a fullscreen black surface with contained image presentation", () => {
  assert.match(stylesSource, /\.reference-preview-overlay\.is-mobile-preview\s*\{[\s\S]*width:\s*100vw;[\s\S]*min-height:\s*100vh;[\s\S]*background:\s*#000;/);
  assert.match(stylesSource, /\.reference-preview-mobile-image-button \.managed-image\s*\{[\s\S]*width:\s*min\(100vw,\s*calc\(\(100vh - 24px\) \* var\(--managed-crop-aspect, 1\)\)\);[\s\S]*max-height:\s*calc\(100vh - 24px\);/);
